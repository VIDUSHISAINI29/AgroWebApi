...
export async function getWheatData(req, res) {
    try {
        const rows = await new Promise((resolve, reject) => {
            ...
            connection.run(
                "CREATE TABLE IF NOT EXISTS Wheat_Data AS SELECT * FROM read_parquet('ParquetFiles/WheatData.parquet')",
                (err) => {
                    if(err){
