...
export async function getSiltSoilData(req, res) {
    try {
        const rows = await new Promise((resolve, reject) => {
            const connection =  db.connect();
            connection.run(
                "CREATE TABLE IF NOT EXISTS SiltSoilData AS SELECT * FROM read_parquet('ParquetFiles/SoilWise/SiltSoilData.parquet')",
                (err) => {
                    if(err){
                    ...
                    connection.all(
                        ...
                        (err, rows) => {
                            connection.close();
                            if(err){
                                console.log("Error in selecting coloumns SiltSoil crops controller", err);
                                return reject(err);
                            }
                            resolve(rows);
                        }
        ...
        console.log("error in controller.js file SiltSoil");
