...
export async function getChalkySoilData(req, res) {
    try {
        const rows = await new Promise((resolve, reject) => {
            const connection =  db.connect();
            connection.run(
                "CREATE TABLE IF NOT EXISTS ChalkySoilData AS SELECT * FROM read_parquet('ParquetFiles/SoilWise/ChalkySoilData.parquet')",
                (err) => {
                    if(err){
        ...
        console.log("error in controller.js file ChalkySoil");
