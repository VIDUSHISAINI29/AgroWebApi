...
export async function getMaizeData(req, res) {
    try {
        const rows = await new Promise((resolve, reject) => {
            ...
            connection.run(
                "CREATE TABLE IF NOT EXISTS Maize_Data AS SELECT * FROM read_parquet('ParquetFiles/CropWise/MaizeData.parquet')",
                (err) => {
                    if(err){
